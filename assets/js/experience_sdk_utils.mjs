...
/* ----end Array Reduce functions---- */

/* Role Functions
==================================
    Allows knowing if a user has certain Experience roles
    Name                Author    Date        Notes
    ------------------  --------  ----------  --------------------------------------------------
    hasRoles             JMM       2025-05-01  Checks if a user has any in a list of roles
    hasRole              JMM       2025-05-01  Checks if a user has a single role

*/
export function hasRoles(props, roleArray) {
  const userRoles = useUserInfo(props).roles;
  const hasRole = roleArray.some((role) => userRoles.includes(role));
  return hasRole;
}

export function hasRole(props, role) {
  const userRoles = useUserInfo(props).roles;
